    ...
    <div className="flex flex-col gap-8 pb-20">
      ...
      <div className="flex flex-col gap-6 px-2">
        <div className="flex justify-between items-end">
          <div className="space-y-1">
            <h2 className="text-xl font-bold text-amber-50/80 tracking-tight">审计账本</h2>
          ...
          <button
            ...
            className="flex items-center gap-2 text-[10px] font-black text-zinc-600 hover:text-red-400 transition-all uppercase tracking-widest py-2 px-4 rounded-xl border border-white/5 hover:bg-red-500/5"
          >
            <Trash2 size={14} />
            <span>销毁审计记录</span>
          </button>
        ...
        <div className="relative w-full max-w-2xl mx-auto group/search">
          ...
          <div className="relative flex items-center h-14 px-5 rounded-2xl transition-all duration-500 border border-amber-500/25 bg-[#141210]/90 shadow-[0_4px_20px_-2px_rgba(0,0,0,0.5)] focus-within:border-amber-500/60 focus-within:bg-black focus-within:shadow-[0_0_25px_rgba(245,158,11,0.08)]">
      ...
      {filteredHistory.length === 0 && !searchQuery && (
        <div className="flex flex-col items-center justify-center py-32 opacity-20">
          ...
          <p className="text-sm font-bold tracking-widest uppercase">暂无脱敏记录</p>
        </div>
      )}
      ...
      {filteredHistory.length === 0 && searchQuery && (
        <div className="flex flex-col items-center justify-center py-32">
          ...
          <p className="text-[10px] text-zinc-600 mt-2">未发现包含 "{searchQuery}" 的审计项</p>
        </div>
      )}
...
function HistoryCard({
  ...
  return (
    <motion.div
      ...
      layout
      className="p-8 rounded-[2.5rem] border border-white/[0.03] bg-[#0c0b0a]/40 hover:bg-[#110f0e]/60 transition-all duration-700 mb-4"
    >
      {/* Meta row */}
      <div className="flex justify-between items-center mb-6">
        <div className="flex items-center gap-4">
          <span className="flex items-center gap-2 text-zinc-500 text-[10px] font-mono font-bold bg-black/40 px-3 py-1.5 rounded-lg border border-white/[0.02]">
            <Clock size={12} /> {item.timestamp}
          </span>

          {item.mode === "SHADOW" ? (
            <span className="flex items-center gap-2 px-3 py-1.5 rounded-lg text-[9px] font-black uppercase border bg-amber-500/10 text-amber-500 border-amber-500/20">
              <Ghost size={11} /> 影子宇宙侦测
            </span>
          ) : (
            <span className="flex items-center gap-2 px-3 py-1.5 rounded-lg text-[9px] font-black uppercase border bg-blue-500/10 text-blue-400 border-blue-500/20">
              <ShieldAlert size={11} /> 哨兵宇宙拦截
            </span>
          )}
        </div>

        <span className="text-[9px] font-mono text-zinc-800 uppercase tracking-widest group-hover/card:text-zinc-600 transition-colors">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 relative">
        ...
        <div className="space-y-3">
          ...
          <div className="p-6 rounded-[1.5rem] text-[13px] font-mono leading-relaxed break-all bg-black/40 text-zinc-500 border-white/[0.03] border h-40 overflow-y-auto custom-scroll">
            {item.original}
          </div>
        </div>
        ...
        <div className="space-y-3">
          ...
          <div className="p-6 rounded-[1.5rem] text-[13px] font-mono leading-relaxed break-all bg-white/[0.01] text-zinc-200 border-white/[0.03] border shadow-inner h-40 overflow-y-auto custom-scroll">
            {item.masked}
          </div>
        </div>
      </div>
...
function CopyButton({
  ...
  return (
    <motion.button
      ...
      whileTap={{ scale: 0.95 }}
      className={cn(
        "flex items-center gap-2 text-[9px] font-bold transition-all px-2.5 py-1.5 rounded-lg bg-white/[0.02] border border-white/[0.05]",
        isCopied
