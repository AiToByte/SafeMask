...
### 方案二：高性能 .NET 流生成器（生成 1000 万行 ≈ 2.7GB）
...
### .NET 流生成器（生成 100 万行 ≈ 280MB）
```powershell
$path = "test_perf_100.log"
$lines = 1000000  # 1000 万行
...
Write-Host "🚀 正在生成 100 万行压力测试数据..." -ForegroundColor Cyan
...
```powershell
# 1. 文件最前面 7 行
Write-Host "=== 文件开头 (前 7 行) ===" -ForegroundColor Cyan
Get-Content output_big.txt -First 7

# 2. 文件最后 7 行
Write-Host "`n=== 文件结尾 (后 7 行) ===" -ForegroundColor Cyan
Get-Content output_big.txt -Tail 7

# 3. 随机抽 6 行（效果比较好）, 大文件不建议使用, 耗时较长
Write-Host "`n=== 文件中随机 6 行 ===" -ForegroundColor Cyan
