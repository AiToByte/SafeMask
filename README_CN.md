...
- **AI NER 实体识别引擎**：基于 ONNX Runtime 的神经网络实体检测（人名、邮箱、电话、密钥等），补充正则/字典匹配的盲区
- **记录持久化**：每次脱敏操作自动写入结构化 Markdown 文件（YAML 头 + 原文/脱敏对照），可用于 AI 训练分析
- **设置即时保存**：所有开关切换后立即同步到 Rust 后端，无需点击保存按钮
- **可配置粘贴快捷键**：在设置页面自定义魔术粘贴的触发快捷键
- **React 19 前端重构**：从 Vue 3 + Pinia 完全重写为 React 19 + Zustand，懒加载组件 + CSS 过渡动画
- **启动性能优化**：关键数据优先加载，非关键 IPC 延后执行，首屏渲染毫秒级完成
...
- **坍缩**：当您准备将内容发给 ChatGPT/Claude 时，按下魔术粘贴快捷键（默认为 `Ctrl+Shift+V`）。SafeMask 瞬间执行 **"备份 → 注入脱敏文 → 模拟粘贴 → 瞬时还原"** 的闪电序列（约 150ms）。
...
- **AI NER (ONNX Runtime)**：神经网络实体识别（人名、邮箱、电话、地址、密钥等），捕获固定规则遗漏的敏感信息。
- **COW (Copy-On-Write) 优化**：如果一行文本未发现隐私，引擎仅返回引用，**不产生任何内存分配**。

### 3. 毫秒级"时间回溯"算法
- **原子锁控制**：使用 Rust `AtomicBool` 协调监听器与执行器，彻底规避模拟粘贴时的"循环脱敏"死锁。
- **注入延迟补偿**：支持 50ms-800ms 的精密延迟调节，确保在高负载应用中也能准确注入。

### 4. AI 训练记录写入器
- 每次脱敏操作自动写入结构化 Markdown 文件（YAML 头 + 原文代码块 + 脱敏代码块 + 实体表 + 统计摘要）。
- 文件自动轮转：每文件 150 条记录，按日期序列命名（如 `2026/2026-07-16-001.md`）。
- 可在设置页面开关；输出目录状态实时显示。
...
- **琥珀象牙主题**：深邃的暖色调布局，配合非对称留白，极大地缓解了长时间盯着屏幕的眼部疲劳。
...
| 快捷键 | 动作 | 语义 |
| :--- | :--- | :--- |
| `Ctrl+Shift+V`（可自定义） | **魔术粘贴** | 将影子宇宙中的脱敏副本注入当前焦点窗口 |
| `Alt + M` | **宇宙切换** | 在"静默监测"与"主动拦截"模式间瞬时切换 |
| 置顶按钮 | **窗口置顶** | 将控制台固定在所有窗口前方 |

---

## 隐私与合规

- **100% 离线**：配置文件中未开启任何网络权限，所有脱敏逻辑完全本地运行。
...
- **前端**: React 19 + Zustand + Vite 6
- **通信**: Tauri v2 IPC (Binary stream)
- **样式**: Tailwind CSS v3 + PostCSS
- **AI 引擎**: ONNX Runtime (ort crate) + HuggingFace tokenizers
- **内存分配器**: mimalloc
- **线程池**: Rayon（限制 2 线程）
- **静态内存**: 约 40MB
...
- **macOS**: 通用二进制 `.dmg` (支持 M1/M2/M3/Intel)
